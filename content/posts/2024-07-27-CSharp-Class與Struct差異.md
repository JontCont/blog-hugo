...
- 參考型別 : 變數儲存的是記憶體位址，`s2 = s1` 共享同一位址，修改 `s2.name` 會同步反映在 `s1` 上，因此 `s1.Equals(s2)` 為 `true`。
- 實質型別 : 變數儲存的是實體數值，`s4 = s3` 會複製一份獨立副本。當 `s4.name` 被修改為 "joh" 後，`s3` 的內容依然為 "conte"，兩者內容不同，因此 `s3.Equals(s4)` 為 `false`。
